...
Three arms, same budget of 12 experiments, 3 seeds:

| arm | E1 best MAE | targets covered | tokens |
|---|---:|---:|---:|
| random sampling | 0.205 | 4 | 0 |
| **fixed human grid** | **0.064** | 3 | **0** |
| agent (mock backend) | 0.075 | 1 | 3,629 |

**The finding here is a bar, not a win.** A 12-line schedule written before any results
were seen beats uniform sampling 3.2× on E1. That is what an LLM arm must clear to have
justified its tokens — a far harder target than "beat random," which is where most agent
evaluations stop.

The mock agent does not clear it. It loses to the grid and spends 3,629 tokens doing so.
That says nothing about LLM agents — the mock is deliberately naive, and exists so the
referee could be tested with no GPU and no network. It says the **measurement can detect a
bad agent**, which is the property that was missing from the original design.
...
- **No real model has run.** Every agent number above is the mock backend. Whether an LLM
  clears the grid bar is unanswered.
